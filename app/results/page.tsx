...
export default function Results() {
  ...
  useEffect(() => {
    async function init() {
      const sRes = await fetch("/api/suggest", { method: "POST", body: JSON.stringify(Object.fromEntries(searchParams)) });
      ...
      setData({ shops, suggestion: sugg, loading: false });
    }
    init();
  }, []);

  if (data.loading) return <div className="min-h-screen flex items-center justify-center animate-pulse">AIが厳選中...</div>;
      ...
      <div className="grid gap-4">
        {data.shops.map((shop: any) => (
          <div key={shop.id} className="bg-white/5 border border-white/10 rounded-3xl overflow-hidden shadow-xl">
            <img src={shop.photo.pc.l} className="h-40 w-full object-cover" />
            <div className="p-4 space-y-2">
              ...
              <a href={shop.urls.pc} target="_blank" className="block w-full bg-white text-black text-center py-3 rounded-xl font-bold mt-2">お店を見る</a>
            </div>
          </div>
        ))}
      </div>
