...
function Content() {
  ...
  if (data.loading) return (
    <div className="min-h-screen bg-white flex flex-col items-center justify-center p-10">
      <div className="w-16 h-16 border-4 border-[#333] rounded-full flex items-center justify-center animate-bounce shadow-[4px_4px_0px_#FFD700]"><Lightbulb size={32}/></div>
      <p className="mt-6 text-xl font-black italic">調査中...</p>
    </div>
  );
    ...
    <main className="min-h-screen bg-[#FDFDFD] p-4 pb-12 max-w-2xl mx-auto text-[#333]">
      ...
      {data.suggestion && (
        <div className="bg-[#FFEE00] border-4 border-[#333] p-6 rounded-[2rem] mb-8 shadow-[6px_6px_0px_#333]">
          <h2 className="text-xl font-black mb-1 flex items-center gap-2"><Sparkles size={18}/> {data.suggestion.keyword}</h2>
          <p className="font-bold text-xs opacity-80">{data.suggestion.reason}</p>
        </div>
      )}
          ...
          <div key={shop.id} className="bg-white border-4 border-[#333] rounded-[2rem] overflow-hidden shadow-[8px_8px_0px_#333]">
            ...
            <div className="p-6 space-y-3">
              ...
              <div className="flex gap-3">
                <div className="bg-[#FFEE00] px-3 py-1 rounded-lg border-2 border-[#333] text-[10px] font-black uppercase">{shop.capacity}席</div>
              </div>
              <a href={shop.urls.pc} target="_blank" rel="noopener noreferrer" className="block w-full bg-[#333] text-white text-center py-4 rounded-xl font-black text-lg shadow-[4px_4px_0px_#FFD700] active:translate-x-1 active:translate-y-1 transition-all">予約する</a>
            </div>
          </div>
        ))}
      </div>
