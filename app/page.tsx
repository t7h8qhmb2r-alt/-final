      ...
      <div className="max-w-md w-full relative z-10 space-y-12">
        <div className="flex flex-col items-center">
          ...
          <div className="text-center font-black">
            <h1 className="text-4xl text-[#333]">2軒目研究所</h1>
            <div className="h-1.5 w-full bg-[#FFD700] rounded-full mt-1"></div>
          </div>
        </div>
        <div className="bg-white p-8 rounded-[2.5rem] border-4 border-[#333] shadow-[10px_10px_0px_#333] space-y-6 text-[#333]">
          <div className="space-y-4">
            <div className="flex items-center gap-4 border-b-4 border-[#333] pb-2">
              <Users size={24} />
              <input type="number" value={formData.members} onChange={e => setFormData({...formData, members: e.target.value})} className="w-full text-2xl font-black outline-none bg-transparent" />
              <span className="font-black">人</span>
            </div>
            <div className="flex items-center gap-4 border-b-4 border-[#333] pb-2">
              <GlassWater size={24} />
              <input type="text" value={formData.previousDrink} onChange={e => setFormData({...formData, previousDrink: e.target.value})} className="w-full text-xl font-black outline-none bg-transparent" placeholder="1軒目で飲んだもの" />
            </div>
          </div>
          <button onClick={handleSearch} disabled={loading} className="w-full h-16 bg-[#FFEE00] border-4 border-[#333] rounded-xl font-black text-xl flex items-center justify-center gap-2 shadow-[4px_4px_0px_#333] active:translate-x-1 active:translate-y-1 active:shadow-none transition-all">
